...
def plot_em(xlabel, ylabel, x_values, y_values, data_label, fig_name):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('./Pressure_analysis/'+fig_name+'.tiff', dpi=300)
    plt.clf()
...
if '-eqnpt' in args:
    ...
    data1 = load_xvg('npt_2.energy1.xvg', column_name1, args[1], args[2], args[3], 30)
    data2 = load_xvg('npt_2.energy2.xvg', column_name2, args[1], args[2], args[3], 28)
    ...
    for i in column_name2[1:]:
        ...
        if i == r'$L_{box}$':
            ...
            for data,pos in zip(y_values,data_num):
                data_L[pos] = data
            np.save('./Pressure_analysis/eqnpt.Lbox.npy', data_L)
